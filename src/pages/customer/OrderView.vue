       ...
       <span >
          {{ orderViewDetails.address.address}}<br>
             ...
             <div class="row py-3 border-bottom fw-semibold text-muted">
               ...
               <div class="row align-items-center py-3 border-bottom" v-for="product in orderViewDetails?.products || []"
  :key="product.id">
                   <div class="col-sm-6">
                   <div class="d-flex align-items-center">
                   <img class="ord_pro_img me-3" :src="image_url+product.product_image" :alt="product.product_name">
               <div>
                <div class="ord_pro_names">{{  product.product_name }}</div>
                    <!-- <div class="ord_pro_type">Skin Type: Oil</div> -->
                   <div class="ord_pro_voll">Item Volume :{{ product.weight }}</div>
                </div>
          </div>
        </div>
               ...
               <div class="col-sm-2 text-end">Rs. {{ product.price * product.qty }}</div>
                ...
                <div class="d-md-none d-block">
                    ...
                    <div class="order_status_wrap">
                        ...
                        <div class="d-flex justify-content-between p-3 ord_pro_details" v-for="product in orderViewDetails?.products || []"
  :key="product.id">
                            <div class="d-flex">
                                <div>
                                    <img class="ord_pro_img" :src="image_url+product.product_image "  :alt="product.product_name ">
                                </div>
                                <div>
                ...
                <div class="mt-4 sub-tol border rounded p-4">
                    ...
                    <div class="">
                      <div class="d-flex justify-content-between mb-2">
                        <span>Sub Total</span>
                        <span>Rs. <span id="subtotal">{{ orderViewDetails?.sub_total }}</span></span>
                 </div>
...
import { getInvoice, getViewOrderDetails } from '../../services/apiService';
...
const userId = atob(localStorage.getItem("user_id"));
 ...
 const dateTime = (date) => {
  ...
  return `${day}-${month}-${year}`;
    ...
    const downloadInvoice = async (code)=>{
        console.log("Api hit");
        
        const billNo = (code).split('#')[1];
        const res = await getInvoice(billNo);
        const blob = new Blob([res.data], { type: 'text/html' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `invoice-${billNo}.html`;
