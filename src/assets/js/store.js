...
const userStore = reactive({
  phone: localStorage.getItem("phone")
    ? atob(localStorage.getItem("phone"))
    : null,
  customerType: localStorage.getItem("customerType")
    ? atob(localStorage.getItem("customerType"))
    : "Both",
...
const fetchwishlistDetails = async (page = 1) => {
  try {
    if (userStore.userId) {
      store.loading = true;
      const orderDetailRes = await getWishlistProduct(userStore.userId, page);
...
const addDeleteWishlist = async (productId, type) => {
  ...
  try {
    ...
    if (type == "delete") {
      store.wishlistProd.data = store.wishlistProd.data.filter(
